...
class Unet(nn.Module):
    ...
    def __init__(self, im_channels, model_config):
        ...
        # text的embedding的维度，这里要改成神经活动embedding的维度
        ...
        if self.condition_config is not None:
            ...
            # 一开始配置文件里没有text_condition_config这一条，要自己设置
        ...
        if self.class_cond:
            ...
            # 这里就是最重要的，把class给embedding进去的操作，但神经活动是多维序列输入，不像类别或者文本。所以把它展平？
        ...
        if self.image_cond:
            ...
            # 是text条件的话，此处只定义进行一个卷积操作
            # 这里的im_channels最后会取值到配置文件里的z_channels
    ...
    def forward(self, x, t, cond_input=None):
        ...
        for idx, down in enumerate(self.downs):
            ...
            # 这里就是一个DownBlock, 这三个参数是在blocks文件里, DownBlock类中的forward方法中定义的
            # 分别是代表输入本身、时间嵌入、文本
